...
float Volume::getSampleTriLinearInterpolation(const glm::vec3& coord) const
{
    ...
    if (glm::any(glm::lessThan(coord, glm::vec3(0))) || glm::any(glm::greaterThanEqual(coord, glm::vec3(m_dim - 1))))
        return 0.0f;
    /*glm::vec3 c000 = glm::vec3(floor(coord.x), floor(coord.y), floor(coord.z));
    glm::vec3 c010 = glm::vec3(floor(coord.x), ceil(coord.y), floor(coord.z));
    glm::vec3 c100 = glm::vec3(ceil(coord.x), floor(coord.y), floor(coord.z));
    glm::vec3 c110 = glm::vec3(ceil(coord.x), ceil(coord.y), floor(coord.z));

    glm::vec3 c001 = glm::vec3(floor(coord.x), floor(coord.y), ceil(coord.z));
    glm::vec3 c011 = glm::vec3(floor(coord.x), ceil(coord.y), ceil(coord.z));
    glm::vec3 c101 = glm::vec3(ceil(coord.x), floor(coord.y), ceil(coord.z));
    glm::vec3 c111 = glm::vec3(ceil(coord.x), ceil(coord.y), ceil(coord.z));*/
...
{
    glm::vec2 c00 = glm::vec2(floor(xyCoord.x), floor(xyCoord.y));
    glm::vec2 c01 = glm::vec2(ceil(xyCoord.x), floor(xyCoord.y));
    glm::vec2 c10 = glm::vec2(floor(xyCoord.x), ceil(xyCoord.y));
    glm::vec2 c11 = glm::vec2(ceil(xyCoord.x), ceil(xyCoord.y));
    float c1 = linearInterpolate(getVoxel(c00.x, c00.y, z), getVoxel(c01.x, c01.y, z), xyCoord.x-c00.x);
    float c2 = linearInterpolate(getVoxel(c10.x, c10.y, z), getVoxel(c11.x, c11.y, z), xyCoord.x - c10.x);
    return linearInterpolate(c1, c2, xyCoord.y - c00.y);
}
