...
{
    // setting maximum iterations to 25 to avoid degenerate cases
    int maxIt = 25;
    int it = 0;
    float val = 0.0f;
    float t = 0.0f;
    ...
    while (it < maxIt) {
        t = (t0 + t1) / 2;
        val = m_pVolume->getSampleInterpolate(ray.origin + t * ray.direction);

        // Check if value at t matches isovalue
        if (abs(val - isoValue) < 0.01) {
            return t;
        }

        // update t0 or t1 based on value at t
        if (val < isoValue) {
            t0 = t;
        } else {
            t1 = t;
...
float Renderer::getTF2DOpacity(float intensity, float gradientMagnitude) const
{
    ...
    float gradientMax = m_pGradientVolume->maxMagnitude();
    float gradientMin = m_pGradientVolume->minMagnitude();
    float sideLen = gradientMax - gradientMin;
    float tan = m_config.TF2DRadius / sideLen;

    float len2Mid;
    if (intensity <= m_config.TF2DIntensity) {
        len2Mid = m_config.TF2DIntensity - intensity;
    } else {
        len2Mid = - m_config.TF2DIntensity + intensity;
    }
    // return 0 for sample lying outside the triangle
    if (len2Mid > tan * gradientMagnitude) {
        return 0.0f;
    }
